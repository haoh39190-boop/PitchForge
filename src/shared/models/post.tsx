...
type LocalMdxData = {
  ...
  version?: string;
  tags?: string[];
  categories?: BlogPostType['categories'];
};
...
export async function getLocalPost({
  ...
  const data = localPost.data as LocalMdxData;
  ...
  const post: BlogPostType = {
    ...
    content: '',
    body: body,
    toc: data.toc, // Use fumadocs auto-generated TOC
...
export async function getLocalPage({
  ...
  const data = localPage.data as LocalMdxData;
  const MDXContent = data.body;
  const body = (
    <MDXContent
      components={getMDXComponents({
...
export async function getLocalPostsAndCategories({
  ...
  localPostsList.push(
    ...localPosts.map((post) => {
      const frontmatter = post.data as LocalMdxData;
      ...
      if (type === PostType.LOG) {
        const MDXContent = frontmatter.body;
